defmodule CentralWeb.Live.Components.Chart.BacktestChartComponent do
  ...
  # Maximum candles to load initially
  @initial_candle_limit 500
  # Size of each historical data chunk on scroll
  @historical_chunk_size 200
  ...
  def update(assigns, socket) do
    ...
    # Need a new backtest to potentially load
    # ID changed
    # Or no data loaded yet
    needs_data_load? =
      new_backtest &&
        (old_backtest_id != new_backtest.id ||
           is_nil(socket.assigns.chart_data))
    ...
    socket =
      if assigns[:trades] &&
           (old_backtest_id != new_backtest.id || socket.assigns[:trades_by_timestamp] == %{}) do
    ...
    if needs_data_load? do
      # Guaranteed to exist due to the check above
      ...
      initial_start_time =
        case TimeframeHelper.timeframe_to_seconds(backtest.timeframe) do
          ...
          seconds when seconds > 0 ->
            calculated_start =
              DateTime.add(backtest.end_time, -seconds * @initial_candle_limit, :second)
          ...
          _ ->
            ...
            Logger.warning(
              "Could not calculate initial start time for backtest chart #{backtest.id} (timeframe: #{backtest.timeframe}). Using full backtest start time."
            )
      ...
      Logger.debug(
        "[BacktestChartComponent] Loading initial data (limit #{@initial_candle_limit}) for backtest ID: #{backtest.id}"
      )

      Logger.debug(
        "[BacktestChartComponent] Query range: #{inspect(initial_start_time)} to #{inspect(backtest.end_time)}"
      )
      ...
      if Enum.empty?(limited_candles) do
        Logger.warning(
          "No candle data found for #{backtest.symbol}/#{backtest.timeframe} from #{inspect(initial_start_time)} to #{inspect(backtest.end_time)}"
        )
      end

      Logger.debug(
        "[BacktestChartComponent] Fetched #{length(limited_candles)} candles with optimized query"
      )
        ...
        if socket.assigns.show_trades do
          filtered_trades =
            filter_trades_for_range(
              socket.assigns.trades_by_timestamp,
              limited_candles,
              @max_trade_batch
            )
  ...
  def handle_event("load-historical-data", payload, socket) do
    ...
    safe_limit =
      if is_integer(limit), do: min(limit, @historical_chunk_size), else: @historical_chunk_size

    case DateTime.from_iso8601(oldest_time_iso) do
      {:ok, oldest_time, _offset} ->
        Logger.debug(
          "[BacktestChartComponent][#{chart_id}] Handling load-historical-data before #{inspect(oldest_time)}, requested limit: #{safe_limit}"
        )
        ...
        historical_candles =
          MarketDataContext.get_candles_with_limit(
            backtest.symbol,
            backtest.timeframe,
            # Use the backtest start time as lower bound
            backtest.start_time,
            # Use the oldest visible candle as upper bound
            oldest_time,
            limit: safe_limit,
            # Get newest candles first (closest to oldest_time)
            order_by: :desc
          )

        if Enum.empty?(historical_candles) do
          Logger.warning(
            "[BacktestChartComponent][#{chart_id}] No historical candles found for range before #{inspect(oldest_time)}"
          )
          ...
          Logger.debug(
            "[BacktestChartComponent][#{chart_id}] Returning #{length(formatted_data)} candles and #{length(formatted_trades)} trades"
          )
      ...
      {:error, reason} ->
        Logger.error(
          "[BacktestChartComponent][#{chart_id}] Invalid oldestTimeISO '#{oldest_time_iso}': #{inspect(reason)}"
        )
  ...
  defp filter_trades_for_range(trades_by_timestamp, candles, limit) do
    if Enum.empty?(candles) || trades_by_timestamp == %{} do
      ...
      matched_trades =
        ...
        # Convert MapSet back to list
        |> MapSet.to_list()

      # Apply limit if specified
      if limit && length(matched_trades) > limit do
        Logger.warning(
          "[BacktestChartComponent] Limiting trades from #{length(matched_trades)} to #{limit}"
        )
  ...
  defp format_trades_for_chart(trades) do
    ...
    if length(formatted) > 0 do
      # Placeholder if needed, or just remove the if block if nothing else goes here
      :ok
    end
