defmodule CentralWeb.StrategyLive.NewFormLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     socket
     # Always use nil for new
     |> assign(json_config_input: default_json_input(nil))
          ...
          case Regex.run(~r/^entry_(indicator|condition|value)_(\d+)$/, key) do
            [_, _field_type, index_str] ->
              ...
              cond do
                ...
                # Skip keys for the removed index
                index == index_to_remove ->
                  acc

                index > index_to_remove ->
                  new_key = String.replace(key, "_\#{index}", "_\#{index - 1}")
                  # Shift keys after the removed index
  ...
  def handle_event("save", params, socket) do
    ...
    case Jason.decode(json_string) do
      ...
      {:error, _reason} ->
        ...
        # Always compare against new strategy default
        default_json = default_json_input(nil)
  ...
  defp default_json_input(nil) do
    ...
    example = %{
      ...
      description:
        "Enters long when RSI < 30 and price crosses above SMA(20). Exits on RSI > 70.",
      config: %{
        timeframe: "1h",
        symbol: "BTCUSDT",
        risk_per_trade: "1.5",
