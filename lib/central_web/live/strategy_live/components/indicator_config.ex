defmodule CentralWeb.StrategyLive.Components.IndicatorConfig do
          ...
          <div>
            <%= if @indicator_id do %>
              ...
              <p class="text-blue-600 text-xs mt-1">
                This indicator will use standard settings designed for most trading scenarios.
              </p>
            <% else %>
              <p class="font-medium">Select an indicator</p>
              <p class="text-blue-600 text-xs mt-1">
                Choose a technical indicator from the dropdown above to configure trading rules.
              </p>
            <% end %>
          </div>
        </div>
  ...
  defp process_indicator_data(indicator_id, params, _params_list) do
    if indicator_id && indicator_id != "" && indicator_id != "nil" do
      ...
      if form_config do
        ...
        processed_params =
          Enum.map(form_config.fields, fn field ->
    ...
    assigns =
      if param_type == :select do
        ...
        select_options =
          Enum.map(options, fn
            {label, value} ->
              %{key: to_string(label), value: to_string(value)}

            value when is_binary(value) ->
              %{key: humanize_value(value), value: value}

            value when is_atom(value) ->
              %{key: humanize_value(Atom.to_string(value)), value: Atom.to_string(value)}

            value ->
              %{key: to_string(value), value: to_string(value)}
          end)
  ...
  defp render_checkbox_input(assigns) do
    ...
    <.form_item>
      <.form_control>
        <.checkbox name={"#{@path}_#{@param_name}"} checked={@param_value === true} value="true" />
        <.form_label>{@param_label}</.form_label>
      </.form_control>
      <%= if @param_description do %>
        <.form_description>
