defmodule CentralWeb.StrategyLive.Components.JsonConfigForm do
  ...
  def render(assigns) do
    ...
    <div id={@id} class="space-y-4">
      ...
      <input type="hidden" name="creation_method" value="json" phx-click="set_json_mode" phx-target={@myself} />
      ...
      <input type="hidden" name="json_is_valid" value={if @json_parse_error, do: "false", else: "true"} />

      <div :if={@json_parse_error} class="bg-destructive/10 border-l-4 border-destructive text-destructive p-4 mb-4" role="alert">
  ...
  def handle_event("update_json_input", params, socket) do
    ...
    {json_parse_error, parsed_json} = case Jason.decode(json_string) do
      {:ok, parsed} ->
      ...
      {:error, %Jason.DecodeError{position: pos, data: data, token: token}} ->
        error_message = "JSON parse error at position #{pos}: unexpected token \"#{token}\" in \"#{String.slice(data, max(0, pos-10), 20)}\""
        {error_message, nil}
    end
    ...
    if socket.assigns[:parent] do
      send(socket.assigns.parent, {:json_config_updated, %{
        input: json_string,
  ...
  defp normalize_json_values(value) when is_nil(value), do: "0" # Replace null with "0"
  ...
  defp normalize_condition(condition) when is_map(condition) do
    ...
    condition = if Map.has_key?(condition, "comparison") && Map.has_key?(condition, "indicator") do
