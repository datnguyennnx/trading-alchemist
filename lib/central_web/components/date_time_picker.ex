defmodule CentralWeb.Components.DateTimePicker do
        ...
        >
          ...
          <div class="flex items-center space-x-2 pt-4 border-t border-border mt-4">
            ...
            <.input
              ...
              placeholder="MM"
            />
            <.select :let={select} id={"#{@id}-period-select"} name={"#{@id}-period"} value={selected_period(@value)} class="period-select w-20" placeholder="AM/PM">
              <.select_trigger builder={select} class="w-full" />
              <.select_content builder={select}>
                <.select_item builder={select} value="AM">AM</.select_item>
                <.select_item builder={select} value="PM">PM</.select_item>
              </.select_content>
            </.select>
          </div>
        </div>
  ...
  defp get_12_hour_format(hour) do
    cond do
      hour == 0 -> 12         # 00:00 -> 12 AM
      hour > 12 -> hour - 12  # 13-23 -> 1-11 PM
      true -> hour            # 1-12 -> 1-12 AM/PM
    end
  end
