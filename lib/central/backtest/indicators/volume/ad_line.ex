defmodule Central.Backtest.Indicators.Volume.AdLine do
  ...
  def ad_line(candles) when is_list(candles) and length(candles) > 0 do
    ...
    mf_volumes = Enum.map(candles, fn candle ->
      # Money Flow Multiplier = ((Close - Low) - (High - Close)) / (High - Low)
      high_low_diff = Decimal.sub(candle.high, candle.low)

      mf_multiplier = if Decimal.equal?(high_low_diff, Decimal.new(0)) do
        Decimal.new(0)  # If high equals low, multiplier is 0
      else
    ...
    {ad_line_values, _} = Enum.map_reduce(mf_volumes, Decimal.new(0), fn mfv, prev_ad ->
  ...
  def chaikin_oscillator(candles, fast_period \\ 3, slow_period \\ 10)
    when is_list(candles) and is_integer(fast_period) and fast_period > 0
    and is_integer(slow_period) and slow_period > 0 do
