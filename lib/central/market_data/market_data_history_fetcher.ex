defmodule Central.MarketData.MarketDataHistoryFetcher do
  ...
  def fetch_range(symbol, timeframe, start_time, end_time) do
    ...
    if is_binary(symbol) and is_binary(timeframe) and
      ...
      Logger.error("Invalid arguments passed to MarketDataHistoryFetcher.fetch_range: #{inspect {symbol, timeframe, start_time, end_time}}")
  ...
  def handle_cast({:fetch_range, symbol, timeframe, start_time, end_time}, state) do
    IO.puts("[MarketDataHistoryFetcher] Handling :fetch_range: #{symbol} #{timeframe} #{inspect start_time} -> #{inspect end_time}")
  ...
  defp do_fetch_and_store(symbol, timeframe, start_time, end_time) do
    ...
    case fetch_binance_data(symbol, timeframe, start_time, end_time) do
      {:ok, binance_candles} ->
        IO.puts("[MarketDataHistoryFetcher] Fetched #{length(binance_candles)} candles from Binance.")
        if Enum.empty?(binance_candles) do
          IO.puts("[MarketDataHistoryFetcher] No candles returned from Binance for range, nothing to store.")
          ...
          IO.puts("[MarketDataHistoryFetcher] Transformed #{length(market_data_changesets)} changesets.")
          ...
          case MarketDataContext.bulk_insert_candles(market_data_changesets) do
            ...
            {:error, :invalid_data_structure} = error ->
              Logger.error("Failed to store candles (invalid data structure) for #{symbol} #{timeframe}")
              IO.puts("[MarketDataHistoryFetcher] Failed to store candles: #{inspect error}")

            {:error, reason} -> # Catch other potential errors
              Logger.error("Failed to store candles for #{symbol} #{timeframe}: #{inspect(reason)}")
              IO.puts("[MarketDataHistoryFetcher] Failed to store candles: #{inspect(reason)}")
          end
        end

      {:error, reason} ->
        Logger.error("Failed to fetch data from Binance for #{symbol} #{timeframe}: #{inspect(reason)}")
  ...
  defp fetch_binance_data(symbol, timeframe, start_time, end_time) do
    Logger.info("Fetching Binance data for #{symbol} #{timeframe} from #{start_time} to #{end_time}")
  ...
  defp broadcast_update(symbol, timeframe, start_time, end_time) do
    ...
    IO.puts("[MarketDataHistoryFetcher] Broadcasting update on topic '#{topic}' with payload: #{inspect payload}")
